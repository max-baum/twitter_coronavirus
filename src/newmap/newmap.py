            ...
            for i, line in enumerate(file):
                ...
                if len(hashtag) > 0:
                    if "text" in hashtag[0].keys():
                        for item in hashtag:
                            htlower = item["text"].lower()
                            if htlower in hashtagsearch:
                                print("list" + str(hashtag) + "item" + str(htlower))
                                lang_counter[(htlower, lang)] += 1
                                country_counter[(htlower, place)] += 1
                                audit = (tweet_id,tweet_text,item["text"], lang, place) 
                                match_audit.append(audit)
                    elif "tag" in hashtag[0].keys():
                        for item in hashtag:
                            htlower = item["tag"].lower()
                            if htlower in hashtagsearch:
                                lang_counter[(htlower, lang)] += 1
                                country_counter[(htlower, place)] += 1
...
filenamelang = 'lang/' + filename_nozip + '.lang'
filenamecountry = 'country/' + filename_nozip + '.country'
filenameaudit = 'audit/' + filename + '.audit'
